...
$petugas_name = $_SESSION['nama_petugas'] ?? 'Petugas';
$napi = mysqli_query($koneksi, "SELECT id_napi, nama_lengkap FROM napi ORDER BY nama_lengkap ASC");
?>
<!DOCTYPE html>
...
<body>
    ...
    <div id="main-wrapper" data-theme="light" data-layout="vertical" data-navbarbg="skin6" data-sidebartype="full" data-sidebar-position="fixed" data-header-position="fixed" data-boxed-layout="full">
        ...
        <div class="page-wrapper">
            ...
            <div class="container-fluid">
                ...
                <div class="card-body">
                    <form action="prosestambahabsensi.php" method="POST" enctype="multipart/form-data">
                    
                        <div class="mb-3">
                            <label>Nama Lengkap</label>
                                <select name="id_napi" id="id_napi" class="form-control select2" required>
                                    <option value="">Pilih Napi</option>
                                    <?php
                                    include 'koneksi.php';
                                    $query = mysqli_query($koneksi, "SELECT id_napi, nama_lengkap FROM napi ORDER BY nama_lengkap ASC");
                                    while ($row = mysqli_fetch_assoc($query)) {
                                        echo "<option value='".$row['id_napi']."'>".$row['nama_lengkap']."</option>";
                                    }
                                    ?>
                                </select>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="jenis_absensi" class="form-control" required>
                                <option value="">-- Pilih Jenis Absesni --</option>
  ...
  <script>
$('#id_napi').change(function() {
    var id_napi = $(this).val();
    if (id_napi) {
        $.ajax({
            url: 'get_napi_detail.php',
            type: 'GET',
            data: { id: id_napi },
            dataType: 'json',
            success: function(napi) {
                console.log("Response dari server:", napi); // debug

                // cek apakah key nya sesuai
                if (napi.blok !== undefined) {
                    $('#blok').val(napi.blok);
                } else {
                    console.log("Key 'blok' tidak ditemukan di JSON");
                }

                if (napi.kamar !== undefined) {
                    $('#kamar').val(napi.kamar);
                } else {
                    console.log("Key 'kamar' tidak ditemukan di JSON");
                }
            },
